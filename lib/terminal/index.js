...
class Terminal {
    ...
    //Resolves path to use for recording of audio / video files
    async getLocalMeetingFilesPath(sessionName) {
        return new Promise((resolve, reject) => {
            try {
                const mountedDrives = []
                fs.readdir(process.env.MEETING_FILES_ROOT_DIR, (err, directories) => {
                    if (err) throw err
                    directories.forEach((directory) => {
                        mountedDrives.push(directory)
                    })
                    // Do i have an attached drive ?
                    if (mountedDrives.length > 0) {
                        //gets first directory as mount path
                        const recordSessionDir = `${process.env.MEETING_FILES_ROOT_DIR}/${mountedDrives[0]}/${sessionName}`
                        try {
                            if (!fs.existsSync(recordSessionDir)) {
                                fs.mkdirSync(recordSessionDir)
                            }
                            return resolve(recordSessionDir)
                        } catch (e) {
                            throw e
                        }
                        // I have no drived attached, using SDcard as failback
                    } else {
                        const recordSessionDir = `${process.env.MEETING_FILES_ROOT_DIR_FAILBACK}/${sessionName}`
                        if (!fs.existsSync(recordSessionDir)) {
                            fs.mkdirSync(recordSessionDir)
                        }
                        return resolve(recordSessionDir)
